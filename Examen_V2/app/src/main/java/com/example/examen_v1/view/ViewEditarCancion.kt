...
class ViewEditarCancion : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        botonGuardarCancion.setOnClickListener {
            ...
            val act = EDatabase.database!!.actualizarCancion(idCancion, nombreCancion, duracionCancion, favoritaCancion, autorCancion, idLista)
            if (act) {
    ...
    private fun llenarCampos() {
        ...
        if (idCancion != -1) {
            val cancion = EDatabase.database!!.consultarCancionPorID(idCancion, idLista)
            if (cancion !== null) {
